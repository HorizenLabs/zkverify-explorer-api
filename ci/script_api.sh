...
docker_image_build_name="${DOCKER_IMAGE_BUILD_NAME:-zkverify-explorer-api}"
docker_hub_org="${DOCKER_HUB_ORG:-horizenlabs}"

docker_writer_password="${DOCKER_WRITER_PASSWORD_API:-}"
...
echo "=== Checking if DOCKER_WRITER_PASSWORD_API is set ==="
if [ -z "${docker_writer_password:-}" ]; then
  fn_die "DOCKER_WRITER_PASSWORD_API variable is not set. Exiting ..."
fi
...
if [ -n "${docker_tag:-}" ]; then
  ...
  docker build -f "ci/Dockerfile_api" \
